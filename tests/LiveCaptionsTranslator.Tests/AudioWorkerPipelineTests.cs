...
public sealed class AudioWorkerPipelineTests
{
    ...
    private sealed class PipelineWorkerFixture
    {
        ...
        public PipelineWorkerFixture(bool failSend = false, bool badSummary = false, bool stallSend = false, TimeSpan sendDelay = default)
        {
        ...
        {
            public IAsrWorkerTransport Create(WorkerPipeIdentity identity) { var value = new PipelineTransport(owner, owner.failSend, owner.badSummary, owner.stallSend, owner.sendDelay); owner.Transports.Add(value); return value; }
        }
    }
    ...
    private sealed class PipelineTransport(PipelineWorkerFixture owner, bool failSend, bool badSummary, bool stallSend, TimeSpan sendDelay) : IAsrWorkerTransport
    {
        ...
        public async Task SendAudioFrameAsync(Guid workerSessionId, NormalizedAudioFrame frame, CancellationToken cancellationToken = default) { if (failSend) throw new WorkerTransportException(AsrWorkerFailureKind.AudioPipeClosed, "audio closed"); if (stallSend) await Task.Delay(Timeout.InfiniteTimeSpan, cancellationToken); if (sendDelay > TimeSpan.Zero) await Task.Delay(sendDelay, cancellationToken); Frames.Add(frame); Events.Add("frame"); }
