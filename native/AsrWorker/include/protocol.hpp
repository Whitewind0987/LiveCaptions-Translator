...
enum class message_type : std::uint16_t { worker_hello=1, host_accept=2, host_reject=3, worker_ready=4, start_audio=5, audio_started=6, stop_audio=7, audio_stopped=8, ping=9, pong=10, worker_status=11, audio_progress=12, error=13, shutdown=14, shutdown_ack=15, caption_event=16, audio_pipe_hello=17, audio_pipe_accepted=18, audio_stream_end=19, audio_frame=100 };
...
struct audio_stream_end { guid worker_session{}, capture_session{}; std::int64_t frames{}, bytes{}, first_sequence{}, final_sequence{}, gaps{}; };
struct stream_statistics { guid worker_session{}, capture_session{}; std::int64_t frames{}, bytes{}, first_sequence{}, last_sequence{}, gaps{}, invalid{}, first_timestamp{}, last_timestamp{}, last_sample_index{}, initial_sequence{1}; };
struct stream_lifecycle { bool streaming{}, ended{}; void start(); void frame() const; void end(); void stop(); };
...
guid parse_guid(std::wstring_view); std::vector<std::uint8_t> parse_hex(std::wstring_view); audio_metadata decode_audio_metadata(std::span<const std::uint8_t>); audio_stream_end decode_audio_stream_end(std::span<const std::uint8_t>); bool validate_audio(stream_statistics&, const audio_metadata&); bool validate_audio_stream_end(const stream_statistics&, const audio_stream_end&); std::vector<std::uint8_t> encode_summary(const stream_statistics&);
std::vector<std::uint8_t> encode_worker_hello(const guid&, std::span<const std::uint8_t>, std::int32_t, std::string_view, std::uint64_t);
std::vector<std::uint8_t> encode_audio_pipe_hello(const guid&, std::span<const std::uint8_t>, std::int32_t);
void decode_audio_pipe_accepted(std::span<const std::uint8_t>, const guid&, std::int32_t);
bool is_known_message_type(std::uint16_t); bool is_optional_unknown(std::uint16_t, std::uint16_t); void accept_sequence(std::uint64_t&, std::uint64_t); void require_correlation(const guid&, const guid&);
}
